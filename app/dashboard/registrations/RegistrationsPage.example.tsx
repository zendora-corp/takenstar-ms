// COMPLETE REGISTRATIONS PAGE IMPLEMENTATION
// Copy this to app/dashboard/registrations/page.tsx
...
export default function RegistrationsPage() {
  ...
  const { getQueryParam, setQueryParams, clearQueryParams } = useQueryState();
  ...
  const { data: schools } = useFetch(districtFilter ? `/api/schools?districtId=${districtFilter}&limit=200` : null);
  ...
  const columns: GridColDef[] = [
    { field: 'full_name', headerName: 'Candidate', width: 200 },
    { field: 'class', headerName: 'Class', width: 80 },
    { field: 'group_type', headerName: 'Group', width: 80 },
    {
      field: 'district',
      headerName: 'District',
      width: 150,
    ...
    { field: 'school_roll_no', headerName: 'Roll No', width: 100 },
    {
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button
            ...
            onClick={() => {
              ...
              setTransactionId(params.row.transaction_id || '');
              setOfflineReceiptNo(params.row.offline_receipt_no || '');
              setPaymentNotes(params.row.payment_notes || '');
    ...
    <Box>
      ...
      <Dialog open={paymentDialogOpen} onClose={() => setPaymentDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <TextField label="Student Name" value={selectedReg?.full_name || ''} disabled fullWidth />
            <TextField label="Payment Option" value={selectedReg?.payment_option || ''} disabled fullWidth />
      ...
      <Drawer anchor="right" open={detailDrawerOpen} onClose={() => setDetailDrawerOpen(false)}>
        <Box sx={{ width: 400, p: 3 }}>
          ...
          {detailReg && (
            <Stack spacing={2}>
              <Box><Typography variant="caption" color="text.secondary">Full Name</Typography><Typography>{detailReg.full_name}</Typography></Box>
              ...
              <Box><Typography variant="caption" color="text.secondary">Class / Group</Typography><Typography>{detailReg.class} / {detailReg.group_type}</Typography></Box>
              ...
              <Box><Typography variant="caption" color="text.secondary">Roll Number</Typography><Typography>{detailReg.school_roll_no}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">Address</Typography><Typography>{detailReg.address}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">Student Mobile</Typography><Typography>{detailReg.student_mobile}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">Guardian Mobile</Typography><Typography>{detailReg.guardian_mobile}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">Email</Typography><Typography>{detailReg.email || 'N/A'}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">Payment Option</Typography><Typography>{detailReg.payment_option}</Typography></Box>
              <Box><Typography variant="caption" color="text.secondary">Payment Status</Typography><Chip label={detailReg.payment_status} size="small" /></Box>
            </Stack>
          )}
        </Box>
      </Drawer>
